...
def download_manga():
    while True:
        ...
        print(f"{cyan}Manga Downloader{reset}: {blue}All new files have been downloaded!{reset}")
        ...
        if wait_time / 60 < 60:
            print(f"{cyan}Manga Downloader{reset}: {red}Restarting in {round(wait_time / 60)} mins.{reset}")
        else:
            print(f"{cyan}Manga Downloader{reset}: {red}Restarting in {round(wait_time / 60 / 60)} hours.{reset}")
